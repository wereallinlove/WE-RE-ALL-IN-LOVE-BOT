...
class Expose(commands.Cog):
    ...
    async def expose(self, interaction: discord.Interaction, user: discord.Member):
        try:
            ...
            if not messages:
                await interaction.followup.send("No juicy messages found to expose them with 😔", ephemeral=True)
                return
            ...
            await interaction.followup.send(f"❌ Error running command: `{e}`", ephemeral=True)
