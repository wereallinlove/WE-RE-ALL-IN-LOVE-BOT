...
def setup(bot):
    @bot.tree.command(name="pin", description="Post an image (JPG, PNG, GIF) to the pins channel.")
    ...
    async def pin(
        ...
        if not history:
            await interaction.followup.send("You must attach an image (JPG, PNG, GIF) to pin.", ephemeral=True)
            return

        file = history[0].attachments[0]

        if not file.content_type or not file.content_type.startswith("image"):
            await interaction.followup.send("Only image files (JPG, PNG, GIF) are allowed. No videos.", ephemeral=True)
            return
