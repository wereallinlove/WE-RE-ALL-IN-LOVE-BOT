...
def setup(bot):
    @bot.tree.command(name="pin", description="Post an image to the pins channel.")
    @app_commands.describe(
        link="A link to the image or the original message",
    ...
    async def pin(
        ...
        if not (link.startswith("http://") or link.startswith("https://")):
            await interaction.response.send_message("You must provide a valid link to an image or message.", ephemeral=True)
            return
