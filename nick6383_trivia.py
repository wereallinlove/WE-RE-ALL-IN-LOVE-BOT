...
class TriviaView(View):
        ...
        if self.message:
            try:
                await self.message.reply(
                    f"<@{self.user_id}> your trivia request expired. Please start a new one.")
    ...
    async def check_answer(self, interaction, choice):
        ...
        if choice == self.correct:
            ...
            embed = discord.Embed(
                title="❌ Incorrect.",
                description=f"You chose **{choice}**.\n\nIncorrect. The correct answer was **{self.correct}**.",
                color=0xe74c3c
            )
...
class NickTrivia(commands.Cog):
    ...
    @commands.command(name="nick6383trivia")
    async def nick6383trivia(self, ctx):
        if ctx.channel.id != TRIVIA_CHANNEL_ID:
            return
        if VERIFIED_ROLE_ID not in [role.id for role in ctx.author.roles]:
            return
        if ctx.author.id in used_recently:
            await ctx.reply("Please wait before using this command again.")
            return
        if ctx.author.id in active_trivia:
            await ctx.reply("You already have a trivia question active!")
            return
        ...
        embed = discord.Embed(
            title=f"Trivia for {ctx.author.display_name}",
            description=f"*{lyric}*",
            color=0xff69b4
        )
        ...
        view = TriviaView(song_title, ctx.author.id)
        ...
        view.message = msg
        active_trivia[ctx.author.id] = msg
