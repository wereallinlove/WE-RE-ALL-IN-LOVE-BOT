...
class Nick6383Trivia(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.trivia_data = [
            # (trivia entries already present here)
        ]
        self.cooldowns = {}  # user_id: timestamp
        self.active_users = set()
        self.stats = {}  # user_id: {'correct': int, 'incorrect': int}
...
},
{
    ...
    async def nick6383trivia(self, interaction: discord.Interaction):
        ...
        if user_id in self.cooldowns and now - self.cooldowns[user_id] < 5:
            await interaction.response.send_message("⏳ You need to wait a few seconds before using this again.", ephemeral=True)
            return
        if user_id in self.active_users:
            await interaction.response.send_message("⚠️ You already have an active trivia question!", ephemeral=True)
            return
        ...
        def make_result_embed(chosen, correct, user_id):
            color = discord.Color.green() if chosen == correct else discord.Color.red()
            title = "✅ Correct!" if chosen == correct else "❌ Incorrect."
            if user_id not in self.stats:
                self.stats[user_id] = {'correct': 0, 'incorrect': 0}
            if chosen == correct:
                self.stats[user_id]['correct'] += 1
                description = f"You got it right!\nCorrect: {self.stats[user_id]['correct']} | Incorrect: {self.stats[user_id]['incorrect']}"
            else:
            ...
            color = discord.Color.green() if chosen == correct else discord.Color.red()
            title = "✅ Correct!" if chosen == correct else "❌ Incorrect."
            description = f"The lyric was from **{correct}**." if chosen != correct else "You got it right!"
            return discord.Embed(title=title, description=description, color=color)
        ...
        class TriviaView(discord.ui.View):
            ...
            async def handle(self, interaction_button: discord.Interaction, chosen):
                ...
                if self.answered:
                    ...
                    return
                self.answered = True
                result_embed = make_result_embed(chosen, correct, interaction.user.id)
